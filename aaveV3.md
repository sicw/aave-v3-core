去中心化的存款、借贷、闪电贷协议. 主要是依靠算法保障系统运行。官网: https://aave.com/
按照从 0 到 1 的思路去看 aaveV3 的协议设计。

1、存款
在银行存款我们将现金存入银行账户, 然后会给我们一个存储凭证。aave 同样的将自己的资金转 aave 地址中。我们将 token 转移到 aave 协议地址就可以了, 对应的给你 1:1 mint token 作为你的存款证明。

```js
    // 1. 将存储资产从user转给aToken的地址
...
在银行存款都是有利息的, 在 aave 中同样有利息, 银行的存款利率是固定的比如: 1 年定期 2% 3 年定期 2.5%等。但是在 aave 中存款的利率是和资金使用率相关的, 也就是与资金池中的钱被借出去多少有关, 借出去的越多, 贷款利息越多, 对应的存储收益越高。所以存储利率是由资金使用率和借款利率决定的。
下图是贷款利率, 分两个阶段。资金的最佳使用率 80%做为分界线(80%的存款都借出去了)。最高贷款利率达 79%(只是某个时刻的, 每次交易贷款利率都会变)。
利率计算:
资金使用率为 40% < 80% 在第一阶段 利率 = 基本利率 + $$\frac{40\%}{80\%}$$ _ 4%
资金使用率为 90% > 80% 在第二阶段 利率 = 基本利率 + 4% + $$\frac{90\% - 80\%}{20\%}$$ _ 75%
...
```
    vars.currentLiquidityRate = _getOverallBorrowRate(  // 借款利率
      ...
      params.averageStableBorrowRate
        ...
        // 资产预留金, 应对客户提款和其他风险, 比如90%的资金使用率用来做贷款
...
在 t3 时刻 用户余额 b3 = m \* $$\frac{p}{q}$$ 这种方式需要合约记录用户在 t1 时刻的 累计流动性指数 所以需要一个 map<address, int256>结构 这也是 aaveV1 版本的存储方式
aaveV1 AToken 代码如下
...
观察上面的 b3 = m _ $$\frac{p}{q}$$, 可以在简化下, 用户在 t1 时刻存入 m 资金, 这时 q 也是知道的。那直接存储$$\frac{m}{q}$$ 的值。在 t3 时刻计算用户余额直接乘以 p 就可以了 b3 = $$\frac{m}{q}$$ _ p 这个$$\frac{m}{q}$$也叫流动性缩放余额。这也是在 aaveV2 aaveV3 中使用的计算方式。这种方式可以去掉 userIndexes 的存储, 降低 gas 费。
aaveV3 代码如下
...
另一方面，在银行贷款需要有一定的条件 稳定的工作，居住地址，民事偿还能力。在 aave 协议中没有这多条件, 只需要你存储的资产价值 > 你要贷款的资产价值即可, 也就是超额抵押。比如你存储了价值 1000eth 的资产, 但是你只能贷款 800eth 的资产。这里存在一个问题，既然是超额抵押 那就说明了我有超过 800eth 的资产，那我为啥还有贷款呢，直接兑换成想要的货币不就行了。
